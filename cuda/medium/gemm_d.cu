...
__global__ void mat_mul (float* __restrict__ C, float* __restrict__ A, float* __restrict__ B, const int M, const int N, const int K) {
    ...
    {
        reinterpret_cast<float4*>(&a_reg)[0] = reinterpret_cast<float4*>(
	    &A[AIterRow * K + AIterCol + Iter * BLOCK_SIZE_K])[0];
	As[AIterCol][AIterRow] = a_reg[0];
	As[AIterCol+1][AIterRow] = a_reg[1];
	As[AIterCol+2][AIterRow] = a_reg[2];
	As[AIterCol+3][AIterRow] = a_reg[3];
	...
	for (int l = 0; l < BLOCK_SIZE_K; ++l)
	{
            reinterpret_cast<float4*>(&a_reg)[0] = reinterpret_cast<float4*>(
	        &As[l][ACalIndex])[0];
	    reinterpret_cast<float4*>(&b_reg)[0] = reinterpret_cast<float4*>(
	        &Bs[l][BCalIndex])[0];
            #pragma unroll
	    for (int i = 0; i < 4; ++i)
	    {
